...
const PublicationsPage: React.FC<PublicationsPageProps> = ({ initialCategory, initialPublicationTitle, onNavigateToPublication }) => {
  ...
  return (
    <div className="pt-24 min-h-screen bg-gray-50 font-sans print:pt-4 print:bg-white">
      ...
      <div className="bg-gray-50 border-b border-gray-100 pb-16 pt-10 relative overflow-hidden print:border-none print:pb-8">
        {/* Background Pattern */}
        ...
        <Reveal delay={100}>
          <div className="flex flex-col md:flex-row justify-between items-end md:items-center mb-12 gap-6 sticky top-24 z-30 bg-white/95 backdrop-blur-md py-4 rounded-3xl px-6 border border-gray-100 shadow-lg transition-all print:hidden">
                  ...
                  <button
                    ...
                    className={`px-5 py-2.5 rounded-full text-sm font-bold transition-all duration-300 active:scale-95 border ${
                      activeTab === category
                        ...
                        : 'bg-white/60 backdrop-blur-md text-gray-600 border-gray-200 hover:border-teal-300 hover:text-teal-700'
                    }`}
                  >
                    {category}
                  </button>
             ...
             <div className="relative w-full md:w-96 group">
                ...
                <div className="absolute -inset-1 bg-gradient-to-r from-teal-200 to-teal-200 rounded-full blur opacity-25 group-hover:opacity-50 transition duration-300"></div>
                <div className="relative">
                   <input 
                      ...
                      className="pl-12 pr-4 py-3.5 bg-white/60 backdrop-blur-md border-2 border-gray-100 rounded-full text-sm font-medium focus:outline-none focus:ring-4 focus:ring-teal-500/10 focus:border-teal-500 w-full shadow-sm hover:shadow-md transition-all placeholder:text-gray-400 text-gray-800"
                   />
           ...
           {isLoading ? (
                ...
                <Reveal key={`${pub.title}-${index}`} delay={index % 3 * 100}>
                  ...
                  >
                     ...
                     <div className={`absolute top-0 left-0 w-full h-1.5 ${
                        pub.type === 'Report' ? 'bg-teal-500' :
                        pub.type === 'Policy Brief' ? 'bg-teal-500' :
                        pub.type === 'News & Insights' ? 'bg-burgundy-500' :
                        'bg-gray-500'
                     }`} />

                     <div className="flex justify-between items-start mb-6">
                        <div className={`p-3.5 rounded-2xl ${
                          pub.type === 'Report' ? 'bg-teal-50 text-teal-600' :
                          pub.type === 'Policy Brief' ? 'bg-teal-50 text-teal-600' :
                          pub.type === 'News & Insights' ? 'bg-burgundy-50 text-burgundy-600' :
                     ...
                     <div className="mt-auto flex items-center justify-between border-t border-gray-50 pt-6">
                        <span className={`text-xs font-bold uppercase tracking-wider ${
                           pub.type === 'Report' ? 'text-teal-700' :
                           pub.type === 'Policy Brief' ? 'text-teal-700' :
                           pub.type === 'News & Insights' ? 'text-burgundy-700' :
                           'text-gray-700'
                        }`}>
                           {pub.type}
                        </span>
             ...
             <div className="col-span-full py-24 text-center">
                ...
                <button 
                  ...
                  className="mt-8 px-6 py-2.5 bg-white/60 backdrop-blur-md border border-gray-300 text-gray-700 font-bold rounded-full hover:bg-gray-50 transition-colors active:scale-95"
                >
                   Clear all filters
                </button>
             </div>
        ...
        {hasMore && (
           <div className="text-center print:hidden">
              <button 
                ...
                className="inline-flex items-center gap-2 px-8 py-3.5 bg-white/60 backdrop-blur-md border border-gray-200 text-gray-700 font-bold rounded-full hover:bg-gray-50 hover:border-teal-200 hover:text-teal-700 transition-all shadow-sm hover:shadow-md active:scale-95 disabled:opacity-70 disabled:cursor-not-allowed"
              >
                 {loadingMore ? (
                   <>
